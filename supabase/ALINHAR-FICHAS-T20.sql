-- =============================================================================
-- ALINHAMENTO: fichas_t20 ↔ frontend (salvarFicha em src/lib/fichas/actions.ts)
-- =============================================================================
-- Execute no SQL Editor do Supabase se aparecer erro do tipo:
--   "Could not find the 'inventario' column of 'fichas_t20' in the schema cache"
--
-- Causa típica: banco criado antes das colunas biografia/inventario (ou script
-- inicial antigo). Este script é idempotente — pode rodar mais de uma vez.
-- =============================================================================

-- Corrige nomes errados (acentos ou camelCase), se existirem
do $$
begin
  if exists (
  ...
  -- coluna existe com tipo inesperado; ajuste manual se necessário
    raise notice 'Coluna biografia existe com tipo diferente de text.';
  end if;
end $$;

-- Garante todas as colunas que o frontend envia no insert/update
...
-- Defaults para linhas antigas (caso colunas tenham sido criadas agora)
...
-- Força o PostgREST (Supabase API) a recarregar o schema cache
